...
public sealed class TerritoryAutoDefenseSystem : EntitySystem
{
    ...
    public override void Update(float frameTime)
    {
        ...
        while (query.MoveNext(out var consoleUid, out var defense, out var console))
        {
            ...
            if (consoleXform.GridUid is not { } ownGrid ||
                ...
                _diplomacy.GetGridFaction(ownGrid) is not { } ownFaction)
            {
                ClearTarget(defense);
                continue;
            }
            ...
            FireTerritoryWeapons(consoleUid, ownGrid, target, targetGrid);
        }
    }
        ...
        out EntityUid? targetGrid)
    {
                 ...
                 _lookup.GetEntitiesInRange<ShipNpcTargetComponent>(origin, defense.Range))
        {
            ...
            if (disposition is TargetDisposition.WarnNeutral or TargetDisposition.EngageNeutral)
                WarnNeutralGrid(consoleUid, candidateGrid, now);
        ...
        out EntityUid targetGrid)
    {
        ...
        if (_diplomacy.GetGridFaction(grid) is not { } targetFaction ||
            !PersistentTerritoryFactions.IsSupported(targetFaction))
        {
            return TargetDisposition.Ignore;
        }

        var relation = _diplomacy.GetRelations(ownFaction, targetFaction);
        if (relation == Relations.Ally)
            return TargetDisposition.Ignore;

        targetGrid = grid;
        if (relation == Relations.War || !defense.WarOnly)
    ...
    private void FireTerritoryWeapons(EntityUid consoleUid, EntityUid ownGrid, EntityUid target, EntityUid targetGrid)
    {
        ...
        var query = EntityQueryEnumerator<PointCannonComponent, GunComponent, TransformComponent>();
        while (query.MoveNext(out var weaponUid, out var cannon, out var gun, out var weaponXform))
        {
            if (weaponXform.GridUid != ownGrid ||
                !cannon.LinkedConsoleIds.Contains(consoleUid))
            {
                continue;
            }
    ...
    private void WarnNeutralGrid(EntityUid consoleUid, EntityUid targetGrid, TimeSpan now)
    {
        ...
        var message = Loc.GetString("territory-auto-defense-neutral-radio-warning", ("vessel", vessel));
